...
public class KWICBackend {
  ...
  private static FlatfileDB database = null; // the flat file database
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      lineManager.setSearchResponseHook(messageDispatcher).setDB(database);
      dataHandler.setInitiateSearchHook(lineManager).setSetLinesHook(lineManager);
      boneMesh.addDataListener(dataHandler);
      
      logHandler.onInfo("BACKEND", "Loading existing entries from disk...");
      lineManager.load();
      ...
      logHandler.onError("BACKEND", "Some exception was thrown: " + e.getMessage());
    }
